# ============================================================
# smartz:ai/swarm/alert — 警报广播（executor = 尸壳，每 20gt 由 tick 调度）
...
execute unless entity @s[tag=sz.alerted] run playsound minecraft:entity.husk.ambient hostile @a ~ ~ ~ 2 0.7
tag @s add sz.alerted
execute on target run tag @s add sz.vip
execute as @e[type=minecraft:husk,tag=sz.init,distance=0.1..40] at @s run function smartz:ai/swarm/respond
tag @e remove sz.vip
